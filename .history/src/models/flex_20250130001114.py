...
# import lightly
# from models.ntxentloss import NTXentLoss

# from models.prompts import brca_prompts, nsclc_prompts, camelyon_prompts, rcc_prompts, prad_prompts, crc_msi_prompts, luad_tp53_prompts, brca_tp53_prompts, stad_msi_prompts, brca_her2_prompts, stad_lauren_prompts
...
class CATEMIL(nn.Module):
    def __init__(self, input_dim=512, n_classes=2, dropout=0.25, act='relu', rrt=None, n_ctx=None, n_flp=0, num_patch_prompt=0, task=None, fold=None, exp_code=None, train=True, base_mil=None, slide_align=1):
        ...
        elif base_mil == 'clam_sb':
            from models.model_clam import CLAM_SB
            self.mil = CLAM_SB(n_classes=n_classes, instance_loss_fn = nn.CrossEntropyLoss())
        elif base_mil == 'transmil':
            from models.transmil import TransMIL
            self.mil = TransMIL(n_classes=n_classes)
        elif base_mil == 'dtfdmil':
            from models.dtfdmil import DTFD_MIL
            self.mil = DTFD_MIL(n_classes=n_classes)
        elif base_mil == 'ilra':
            from models.ilra import ILRA
            self.mil = ILRA(n_classes=n_classes)
        elif base_mil == 'dsmil':
            from models.model_dsmil import MILNet
            self.mil = MILNet(n_classes=n_classes)
        elif base_mil == 'acmil':
            from models.acmil import ACMIL_GA
            self.mil = ACMIL_GA(n_classes=n_classes)
        ...
        if train:
            ...
            if task == 'BRCA':
                ...
                from models.prompts import brca_tp53_prompts
                prompts = brca_tp53_prompts()
                ...
                from models.prompts import brca_egfr_prompts
                prompts = brca_egfr_prompts()

            elif task == 'BRCA_BRCA1':
                from models.prompts import brca_brca1_prompts
                prompts = brca_brca1_prompts()

            elif task == 'BRCA_BRCA2':
                from models.prompts import brca_brca2_prompts
                prompts = brca_brca2_prompts()

            elif task == 'BRCA_ESR1':
                from models.prompts import brca_esr1_prompts
                prompts = brca_esr1_prompts()

            elif task == 'BRCA_GATA3':
                from models.prompts import brca_gata3_prompts
                prompts = brca_gata3_prompts()


            elif task == 'LUAD_TP53':

                from models.prompts import luad_tp53_prompts
                prompts = luad_tp53_prompts()

                visual_prompts_path = './prmpts/luad_tp53'
                vis_feats_pos = []
                vis_feats_neg = []
                for file in os.listdir(visual_prompts_path+'/pos'):
                    image = Image.open(visual_prompts_path+'/pos/'+file)
                ...
                from models.prompts import luad_kras_prompts
                prompts = luad_kras_prompts()

            elif task == 'LUAD_TTN':
                from models.prompts import luad_ttn_prompts
                prompts = luad_ttn_prompts()

                visual_prompts_path = './prmpts/luad_ttn'
                vis_feats_pos = []
                vis_feats_neg = []
                for file in os.listdir(visual_prompts_path+'/pos'):
                    image = Image.open(visual_prompts_path+'/pos/'+file)
